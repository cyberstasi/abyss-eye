...
async def get_ship_attributes(ship_name: str) -> dict:
    ...
    try:
        res = await post(f"{ESI_BASE}/universe/ids/", json=[ship_name], timeout=10.0)
        if res.status_code != 200:
            return {"error": f"Could not resolve ship name: {ship_name}"}

        types = res.json().get("inventory_types", [])
        if not types:
            return {"error": f"Ship not found: {ship_name}"}

        type_id = types[0]["id"]

        dogma_res = await get(f"{ESI_BASE}/universe/types/{type_id}/", timeout=10.0)
...
def validate_fit(ship_attrs: dict, high_slots: list, mid_slots: list,
    ...
    """Validate a fit against ship attributes. Returns a list of warnings."""
    warnings = []
